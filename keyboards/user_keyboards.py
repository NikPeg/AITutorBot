...
def user_t_markup(numb):
    markup = InlineKeyboardMarkup(resize_keyboard=True, row_width=True)
    markup.add(
        InlineKeyboardButton(text="✈️Отправить решение", callback_data=f"send_{numb}"),
...
def t_check_markup(user_id, numb):
    markup = InlineKeyboardMarkup(resize_keyboard=True, row_width=True)
    markup.add(
        InlineKeyboardButton(text="👍Отправить проверку студенту", callback_data=f"allow_{user_id}_{numb}"),
...
def t_menu():
    markup = InlineKeyboardMarkup(resize_keyboard=True, row_width=True)
    markup.add(
        InlineKeyboardButton(text="👨‍🎓Список студентов", callback_data="all"),
